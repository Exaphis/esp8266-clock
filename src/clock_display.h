...
class ClockDisplay {
   public:
    ClockDisplay(FastLED_NeoMatrix& matrix) : matrix(matrix) {
   ...
   private:
    ...
    void drawDayProgress(float progress, uint16_t src_color, uint16_t dst_color, int num_skipped_pixels = 0);
    void printTime(int hour, int minute);
    void playSnakeAnimation(uint16_t color);
